  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [
        ...
        codePracticeApi.getUserProgress().catch(() => ({ data: null })),
      ]);
      ...
      // Extract code practice data
      const codeProgress = codeProgressData?.data;
      const studyHours = codeProgress?.totalTimeSpent
        ? Math.floor(codeProgress.totalTimeSpent / 60)
        : 0;
      ...
      setStats({
        ...
        studyHours,
        xp: rankData.xp || 0,
        problemsSolved: codeProgress?.totalProblemsSolved || 0,
        codingStreak: codeProgress?.currentStreak || 0,
      });
    } catch (error) {
        ...
        <div className="grid gap-4 md:grid-cols-5">
          ...
          >
            ...
            <CardContent>
              <div
                className="text-2xl font-bold"
                data-testid="assignments-due-count"
              >
                {loading ? (
                  <Loader2 className="h-6 w-6 animate-spin" />
                ) : (
          ...
          <Card
            ...
            data-testid="problems-solved-card"
          >
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium">
                Problems Solved
            ...
            <CardContent>
              <div className="text-2xl font-bold" data-testid="problems-solved">
                {loading ? (
                  <Loader2 className="h-6 w-6 animate-spin" />
                ) : (
          ...
          <Card
            className="stat-card-orange border-white/10 cursor-pointer hover:border-primary/50 transition-colors"
            onClick={() => navigate("/code-practice")}
            data-testid="coding-streak-card"
          >
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium">
                Coding Streak
            ...
            <CardContent>
              <div className="text-2xl font-bold" data-testid="coding-streak">
                {loading ? (
                  <Loader2 className="h-6 w-6 animate-spin" />
                ) : (
                  stats.codingStreak
